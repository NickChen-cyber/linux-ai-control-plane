...
function MaintenanceWindows({canManage,hosts,rules}:{canManage:boolean;hosts:HostRow[];rules:AlertRule[]}){
 const [windows,setWindows]=useState<any[]>([]);const [open,setOpen]=useState(false);const [error,setError]=useState("");
 const load=useCallback(async()=>{const r=await fetch("/api/maintenance-windows",{cache:"no-store"});const b=await r.json();if(!r.ok)throw new Error(b.detail||"載入失敗");setWindows(b.windows||[])},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);
 const create=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const d=new FormData(e.currentTarget);const r=await fetch("/api/maintenance-windows",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:d.get("name"),hostId:d.get("hostId")||null,ruleId:d.get("ruleId")||null,startsAt:new Date(String(d.get("startsAt"))).toISOString(),endsAt:new Date(String(d.get("endsAt"))).toISOString(),reason:d.get("reason"),suppressNotifications:d.get("suppressNotifications")==="on",pauseEscalations:d.get("pauseEscalations")==="on"})});const b=await r.json();if(!r.ok){setError(b.detail||"建立失敗");return}setWindows(b.windows||[]);setOpen(false)};
 const remove=async(id:string)=>{const r=await fetch(`/api/maintenance-windows/${id}`,{method:"DELETE"});if(!r.ok){setError("刪除失敗");return}await load()};
 return <div className="card maintenance-windows"><header className="alert-section-head"><div><small>PLANNED MAINTENANCE</small><h2>維護時段管理</h2><p>持續採集與保存告警證據，只暫停外部通知及再次提醒。</p></div>{canManage&&<button className="create" onClick={()=>setOpen(true)}>＋ 安排維護</button>}</header>{error&&<div className="log-error">{error}</div>}<div className="data-table"><table><thead><tr><th>名稱</th><th>範圍</th><th>開始</th><th>結束</th><th>通知／升級</th><th>狀態</th><th>操作</th></tr></thead><tbody>{windows.map(item=><tr key={item.id}><td><strong>{item.name}</strong><small>{item.reason}</small></td><td>{hosts.find(h=>h.id===item.hostId)?.name||"全部主機"} · {rules.find(r=>r.id===item.ruleId)?.name||"全部規則"}</td><td>{new Date(item.startsAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{new Date(item.endsAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{item.suppressNotifications?"抑制":"照常"}／{item.pauseEscalations?"暫停":"照常"}</td><td><span className={`channel-state ${item.active?"enabled":"disabled"}`}>{item.active?"維護中":item.upcoming?"即將開始":"已結束"}</span></td><td>{canManage&&<button onClick={()=>void remove(item.id)}>刪除</button>}</td></tr>)}</tbody></table></div>{!windows.length&&<div className="empty-state"><strong>目前沒有維護時段</strong></div>}{open&&<div className="modal-shell"><form className="modal maintenance-modal" onSubmit={create}><header><div><small>MAINTENANCE WINDOW</small><h2>安排維護時段</h2></div><button type="button" onClick={()=>setOpen(false)}>×</button></header><label>名稱<input name="name" defaultValue="系統更新維護" required/></label><label>主機<select name="hostId"><option value="">全部主機</option>{hosts.map(h=><option key={h.id} value={h.id}>{h.name}</option>)}</select></label><label>告警規則<select name="ruleId"><option value="">全部規則</option>{rules.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select></label><label>開始<input name="startsAt" type="datetime-local" required/></label><label>結束<input name="endsAt" type="datetime-local" required/></label><label>原因<input name="reason" defaultValue="計畫性更新與重新啟動" required/></label><label className="inline-check"><input name="suppressNotifications" type="checkbox" defaultChecked/><span>抑制外部通知</span></label><label className="inline-check"><input name="pauseEscalations" type="checkbox" defaultChecked/><span>暫停再次提醒與重大升級</span></label><footer><button type="button" onClick={()=>setOpen(false)}>取消</button><button className="create">建立時段</button></footer></form></div>}</div>
}

function AlertInhibitions({canManage,rules}:{canManage:boolean;rules:AlertRule[]}){
 const [items,setItems]=useState<any[]>([]);const [editing,setEditing]=useState<any|null>(null);const [open,setOpen]=useState(false);const [error,setError]=useState("");
 const load=useCallback(async()=>{const r=await fetch("/api/alert-inhibitions",{cache:"no-store"});const b=await r.json();if(!r.ok)throw new Error(b.detail||"載入失敗");setItems(b.inhibitions||[])},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);const begin=(item:any|null)=>{setEditing(item);setOpen(true)};
 const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const d=new FormData(e.currentTarget);const payload={name:d.get("name"),sourceRuleId:d.get("sourceRuleId"),targetRuleId:d.get("targetRuleId"),reason:d.get("reason"),enabled:d.get("enabled")==="on"};const r=await fetch(editing?`/api/alert-inhibitions/${editing.id}`:"/api/alert-inhibitions",{method:editing?"PUT":"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});const b=await r.json();if(!r.ok){setError(b.detail||"儲存失敗");return}setItems(b.inhibitions||[]);setOpen(false)};
 const remove=async(id:string)=>{const r=await fetch(`/api/alert-inhibitions/${id}`,{method:"DELETE"});if(!r.ok){setError("刪除失敗");return}await load()};
 return <div className="card inhibition-center"><header className="alert-section-head"><div><small>ROOT CAUSE INHIBITION</small><h2>告警相依與抑制</h2><p>根因告警作用期間，只抑制同一台主機的目標通知與升級，子告警證據仍保留。</p></div>{canManage&&<button className="create" onClick={()=>begin(null)}>＋ 新增相依</button>}</header>{error&&<div className="log-error">{error}</div>}<div className="data-table"><table><thead><tr><th>名稱</th><th>根因規則</th><th></th><th>被抑制規則</th><th>原因</th><th>狀態</th><th>操作</th></tr></thead><tbody>{items.map(item=><tr key={item.id}><td><strong>{item.name}</strong></td><td>{item.sourceRuleName}</td><td>→</td><td>{item.targetRuleName}</td><td>{item.reason}</td><td>{item.enabled?"啟用":"停用"}</td><td>{canManage&&<div className="access-actions"><button onClick={()=>begin(item)}>修改</button><button onClick={()=>void remove(item.id)}>刪除</button></div>}</td></tr>)}</tbody></table></div>{!items.length&&<div className="empty-state"><strong>尚無告警相依規則</strong><small>可設定「主機離線 → 抑制 CPU／RAM／磁碟」減少通知風暴。</small></div>}{open&&<div className="modal-shell"><form className="modal inhibition-modal" onSubmit={save}><header><div><small>INHIBITION RULE</small><h2>{editing?"修改相依規則":"新增相依規則"}</h2></div><button type="button" onClick={()=>setOpen(false)}>×</button></header><label>名稱<input name="name" defaultValue={editing?.name||"離線時抑制資源告警"} required/></label><label>根因規則<select name="sourceRuleId" defaultValue={editing?.sourceRuleId||rules.find(r=>r.metric==="availability")?.id||""} required>{rules.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select></label><label>被抑制規則<select name="targetRuleId" defaultValue={editing?.targetRuleId||rules.find(r=>r.metric==="cpu")?.id||""} required>{rules.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select></label><label>原因<input name="reason" defaultValue={editing?.reason||"主機離線是後續資源異常的根因"} required/></label><label className="inline-check"><input name="enabled" type="checkbox" defaultChecked={editing?.enabled??true}/><span>啟用此相依規則</span></label><footer><button type="button" onClick={()=>setOpen(false)}>取消</button><button className="create">儲存規則</button></footer></form></div>}</div>
}

function RootCauseCorrelations(){
 const [items,setItems]=useState<any[]>([]);const [error,setError]=useState("");const load=useCallback(async()=>{const r=await fetch("/api/alert-correlations",{cache:"no-store"});const b=await r.json();if(!r.ok)throw new Error(b.detail||"載入失敗");setItems(b.correlations||[])},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);
 const active=items.filter(item=>item.status==="active");
 return <div className="card correlation-center"><header className="alert-section-head"><div><small>ROOT CAUSE CORRELATION</small><h2>根因關聯視圖</h2><p>顯示相依規則實際命中的根因與子告警，不只是規則設定。</p></div><div className="correlation-summary"><strong>{active.length}</strong><small>作用中關聯</small><button className="secondary-action" onClick={()=>void load()}>重新整理</button></div></header>{error&&<div className="log-error">{error}</div>}<div className="correlation-list">{items.map(item=><article key={item.id} className={item.status}><header><div><strong>{item.hostName}</strong><small>{item.inhibitionName}</small></div><span className={`channel-state ${item.status==="active"?"enabled":"disabled"}`}>{item.status==="active"?"作用中":"已解除"}</span></header><div className="correlation-flow"><section><small>ROOT CAUSE</small><strong>{item.rootRuleName}</strong><p>{item.rootMessage}</p></section><b>→</b><section><small>INHIBITED CHILD</small><strong>{item.childRuleName}</strong><p>{item.childMessage}</p></section></div><footer><span>首次：{new Date(item.firstSeenAt).toLocaleString("zh-TW",{hour12:false})}</span><span>{item.releasedAt?`解除：${new Date(item.releasedAt).toLocaleString("zh-TW",{hour12:false})}`:`最後命中：${new Date(item.lastSeenAt).toLocaleString("zh-TW",{hour12:false})}`}</span></footer></article>)}</div>{!items.length&&<div className="empty-state"><strong>尚無實際根因關聯</strong><small>相依規則命中並抑制通知後，會在此建立永久紀錄。</small></div>}</div>
}

function AlertStormProtection({canManage}:{canManage:boolean}){
 const [report,setReport]=useState<any|null>(null);const [editing,setEditing]=useState(false);const [error,setError]=useState("");const load=useCallback(async()=>{const r=await fetch("/api/alert-storms",{cache:"no-store"});const b=await r.json();if(!r.ok)throw new Error(b.detail||"載入失敗");setReport(b)},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);
 const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const d=new FormData(e.currentTarget);const r=await fetch("/api/alert-storms/policy",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({enabled:d.get("enabled")==="on",windowMinutes:Number(d.get("windowMinutes")),eventThreshold:Number(d.get("eventThreshold")),cooldownMinutes:Number(d.get("cooldownMinutes"))})});const b=await r.json();if(!r.ok){setError(b.detail||"儲存失敗");return}setReport(b);setEditing(false)};
 return <div className="card storm-center"><header className="alert-section-head"><div><small>ALERT STORM PROTECTION</small><h2>告警風暴保護</h2><p>達門檻時先送一則風暴摘要，冷卻期間保留事件但抑制後續個別通知。</p></div><div className="reliability-actions">{canManage&&<button className="secondary-action" onClick={()=>setEditing(true)}>調整政策</button>}<button className="create" onClick={()=>void load()}>重新整理</button></div></header>{error&&<div className="log-error">{error}</div>}{report&&<><div className="storm-policy-summary"><span><small>功能</small><strong>{report.policy.enabled?"啟用":"停用"}</strong></span><span><small>時間窗</small><strong>{report.policy.windowMinutes} 分鐘</strong></span><span><small>觸發門檻</small><strong>{report.policy.eventThreshold} 事件</strong></span><span><small>冷卻時間</small><strong>{report.policy.cooldownMinutes} 分鐘</strong></span></div><div className="data-table"><table><thead><tr><th>主機</th><th>事件數</th><th>開始</th><th>最後事件</th><th>摘要通知</th><th>狀態</th></tr></thead><tbody>{report.storms.map((item:any)=><tr key={item.id}><td><strong>{item.hostName}</strong><small>{item.id}</small></td><td>{item.eventCount}</td><td>{new Date(item.startedAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{new Date(item.lastEventAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{item.summarySentAt?new Date(item.summarySentAt).toLocaleString("zh-TW",{hour12:false}):"—"}</td><td><span className={`channel-state ${item.status==="active"?"enabled":"disabled"}`}>{item.status==="active"?"保護中":"已結束"}</span></td></tr>)}</tbody></table></div></>}{!report?.storms.length&&<div className="empty-state"><strong>尚無告警風暴</strong></div>}{editing&&report&&<div className="modal-shell"><form className="modal storm-policy" onSubmit={save}><header><div><small>STORM CONTROL POLICY</small><h2>調整風暴保護</h2></div><button type="button" onClick={()=>setEditing(false)}>×</button></header><label className="inline-check"><input name="enabled" type="checkbox" defaultChecked={report.policy.enabled}/><span>啟用告警風暴保護</span></label><label>統計時間窗（分鐘）<input name="windowMinutes" type="number" min="1" max="60" defaultValue={report.policy.windowMinutes}/></label><label>觸發事件數<input name="eventThreshold" type="number" min="2" max="100" defaultValue={report.policy.eventThreshold}/></label><label>冷卻時間（分鐘）<input name="cooldownMinutes" type="number" min="1" max="240" defaultValue={report.policy.cooldownMinutes}/></label><footer><button type="button" onClick={()=>setEditing(false)}>取消</button><button className="create">儲存政策</button></footer></form></div>}</div>
}

function OnCallSchedule({canManage}:{canManage:boolean}){
 const [data,setData]=useState<any>({users:[],shifts:[],assignments:[]});const [open,setOpen]=useState(false);const [error,setError]=useState("");const responseBody=async(r:Response)=>{const text=await r.text();try{return JSON.parse(text)}catch{return {detail:r.ok?"伺服器回應格式錯誤":`伺服器錯誤（HTTP ${r.status}）`}}};const load=useCallback(async()=>{const r=await fetch("/api/on-call",{cache:"no-store"});const b=await responseBody(r);if(!r.ok)throw new Error(b.detail||"載入失敗");setData(b);setError("")},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);
 const create=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const d=new FormData(e.currentTarget);const r=await fetch("/api/on-call/shifts",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({userId:d.get("userId"),startsAt:new Date(String(d.get("startsAt"))).toISOString(),endsAt:new Date(String(d.get("endsAt"))).toISOString(),note:d.get("note"),enabled:true})});const b=await responseBody(r);if(!r.ok){setError(b.detail||"建立失敗");return}setData(b);setOpen(false)};const remove=async(id:string)=>{const r=await fetch(`/api/on-call/shifts/${id}`,{method:"DELETE"});if(!r.ok){setError(`刪除失敗（HTTP ${r.status}）`);return}await load()};const current=data.shifts.find((s:any)=>s.active);
 return <div className="card oncall-center"><header className="alert-section-head"><div><small>ON-CALL OWNERSHIP</small><h2>值班排程與自動指派</h2><p>新告警自動指派給目前值班人員，並保存永久指派歷史。</p></div>{canManage&&<button className="create" onClick={()=>setOpen(true)}>＋ 安排值班</button>}</header>{error&&<div className="log-error">{error}</div>}<div className="oncall-current"><span><small>目前值班</small><strong>{current?.displayName||"尚無值班人員"}</strong><em>{current?`至 ${new Date(current.endsAt).toLocaleString("zh-TW",{hour12:false})}`:"新告警維持未指派"}</em></span><span><small>未來／歷史班次</small><strong>{data.shifts.length}</strong></span><span><small>自動指派紀錄</small><strong>{data.assignments.length}</strong></span></div><div className="data-table"><table><thead><tr><th>值班人員</th><th>開始</th><th>結束</th><th>備註</th><th>狀態</th><th>操作</th></tr></thead><tbody>{data.shifts.map((item:any)=><tr key={item.id}><td><strong>{item.displayName}</strong><small>@{item.username}</small></td><td>{new Date(item.startsAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{new Date(item.endsAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{item.note||"—"}</td><td><span className={`channel-state ${item.active?"enabled":"disabled"}`}>{item.active?"值班中":new Date(item.startsAt)>new Date()?"待值班":"已結束"}</span></td><td>{canManage&&<button onClick={()=>void remove(item.id)}>刪除</button>}</td></tr>)}</tbody></table></div>{data.assignments.length>0&&<div className="assignment-history">{data.assignments.slice(0,10).map((item:any)=><span key={item.id}><strong>{item.displayName}</strong><small>{item.hostName} · {item.ruleName}</small><time>{new Date(item.assignedAt).toLocaleString("zh-TW",{hour12:false})}</time></span>)}</div>}{open&&<div className="modal-shell"><form className="modal oncall-modal" onSubmit={create}><header><div><small>ON-CALL SHIFT</small><h2>安排值班</h2></div><button type="button" onClick={()=>setOpen(false)}>×</button></header><label>值班人員<select name="userId" required>{data.users.map((u:any)=><option key={u.id} value={u.id}>{u.displayName} · @{u.username}</option>)}</select></label><label>開始<input name="startsAt" type="datetime-local" required/></label><label>結束<input name="endsAt" type="datetime-local" required/></label><label>備註<input name="note" defaultValue="平台告警值班"/></label><footer><button type="button" onClick={()=>setOpen(false)}>取消</button><button className="create">建立班次</button></footer></form></div>}</div>
}

function OnCallHandoffs({canManage}:{canManage:boolean}){const [data,setData]=useState<any>({users:[],shifts:[],history:[]});const [open,setOpen]=useState(false);const [error,setError]=useState("");const load=useCallback(async()=>{const r=await fetch("/api/on-call-handoffs",{cache:"no-store"});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.detail||`載入失敗（HTTP ${r.status}）`);setData(b);setError("")},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch("/api/on-call-handoffs",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({shiftId:f.get("shiftId"),toUserId:f.get("toUserId"),reason:f.get("reason")})});const b=await r.json().catch(()=>({}));if(!r.ok){setError(b.detail||"交接失敗");return}setData(b);setOpen(false)};return <div className="card handoff-center"><header className="alert-section-head"><div><small>ON-CALL HANDOFF</small><h2>值班交接與代理</h2><p>將作用中或未來班次正式轉交，並永久保存交接責任鏈。</p></div>{canManage&&data.shifts.length>0&&<button className="create" onClick={()=>setOpen(true)}>＋ 建立交接</button>}</header>{error&&<div className="log-error">{error}</div>}<div className="data-table"><table><thead><tr><th>原值班人</th><th>代理人</th><th>原因</th><th>操作人</th><th>交接時間</th></tr></thead><tbody>{data.history.map((x:any)=><tr key={x.id}><td>{x.fromName||"已刪除使用者"}</td><td><strong>{x.toName||"已刪除使用者"}</strong></td><td>{x.reason}</td><td>{x.actorName||"系統"}</td><td>{new Date(x.handedOffAt).toLocaleString("zh-TW",{hour12:false})}</td></tr>)}</tbody></table></div>{!data.history.length&&<div className="empty-state"><strong>尚無值班交接紀錄</strong></div>}{open&&<div className="modal-shell"><form className="modal oncall-modal" onSubmit={save}><header><div><small>SHIFT HANDOFF</small><h2>建立值班交接</h2></div><button type="button" onClick={()=>setOpen(false)}>×</button></header><label>交接班次<select name="shiftId" required>{data.shifts.map((s:any)=><option key={s.id} value={s.id}>{s.displayName} · {new Date(s.startsAt).toLocaleString("zh-TW",{hour12:false})}</option>)}</select></label><label>代理人<select name="toUserId" required>{data.users.map((u:any)=><option key={u.id} value={u.id}>{u.displayName} · @{u.username}</option>)}</select></label><label>交接原因<textarea name="reason" minLength={2} maxLength={500} required placeholder="例如：臨時請假、跨班支援"/></label><footer><button type="button" onClick={()=>setOpen(false)}>取消</button><button className="create">確認交接</button></footer></form></div>}</div>}

function OnCallCoverage({canManage}:{canManage:boolean}){const [data,setData]=useState<any|null>(null);const [editing,setEditing]=useState(false);const [error,setError]=useState("");const load=useCallback(async()=>{const r=await fetch("/api/on-call-coverage",{cache:"no-store"});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.detail||`載入失敗（HTTP ${r.status}）`);setData(b);setError("")},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch("/api/on-call-coverage/policy",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({horizonHours:Number(f.get("hours")),targetPercent:Number(f.get("target"))})});const b=await r.json().catch(()=>({}));if(!r.ok){setError(b.detail||"儲存失敗");return}setData(b);setEditing(false)};return <div className="card coverage-center"><header className="alert-section-head"><div><small>ON-CALL COVERAGE</small><h2>值班覆蓋率與缺口</h2><p>合併重疊班次並找出未來排班的無人值班區段。</p></div><div>{canManage&&<button onClick={()=>setEditing(true)}>調整目標</button>}<button className="create" onClick={()=>void load()}>重新計算</button></div></header>{error&&<div className="log-error">{error}</div>}{data&&<><div className="ownership-kpis"><span><small>觀察範圍</small><strong>{data.policy.horizonHours} 小時</strong></span><span><small>覆蓋率</small><strong className={data.summary.targetMet?"ok":"warn"}>{data.summary.coveragePercent}%</strong></span><span><small>目標</small><strong>{data.policy.targetPercent}%</strong></span><span><small>缺口</small><strong>{data.summary.gapCount}</strong></span><span><small>未覆蓋</small><strong>{data.summary.uncoveredMinutes} 分鐘</strong></span></div><div className="data-table"><table><thead><tr><th>缺口開始</th><th>缺口結束</th><th>持續時間</th></tr></thead><tbody>{data.gaps.map((g:any,i:number)=><tr key={`${g.startsAt}-${i}`}><td>{new Date(g.startsAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{new Date(g.endsAt).toLocaleString("zh-TW",{hour12:false})}</td><td><strong>{g.durationMinutes} 分鐘</strong></td></tr>)}</tbody></table></div>{!data.gaps.length&&<div className="empty-state"><strong>觀察範圍內沒有值班缺口</strong></div>}{editing&&<div className="modal-shell"><form className="modal oncall-modal" onSubmit={save}><header><div><small>COVERAGE OBJECTIVE</small><h2>調整值班覆蓋目標</h2></div><button type="button" onClick={()=>setEditing(false)}>×</button></header><label>觀察範圍（小時）<input name="hours" type="number" min="1" max="720" defaultValue={data.policy.horizonHours}/></label><label>覆蓋率目標（%）<input name="target" type="number" min="1" max="100" step="0.01" defaultValue={data.policy.targetPercent}/></label><footer><button type="button" onClick={()=>setEditing(false)}>取消</button><button className="create">儲存目標</button></footer></form></div>}</>}</div>}

function OnCallGapAlerts({canManage}:{canManage:boolean}){const [data,setData]=useState<any|null>(null);const [editing,setEditing]=useState(false);const [error,setError]=useState("");const load=useCallback(async()=>{const r=await fetch("/api/on-call-coverage",{cache:"no-store"});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.detail||"載入失敗");setData(b);setError("")},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch("/api/on-call-coverage/policy",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({horizonHours:data.policy.horizonHours,targetPercent:data.policy.targetPercent,alertEnabled:f.get("enabled")==="on",alertLeadHours:Number(f.get("lead"))})});const b=await r.json().catch(()=>({}));if(!r.ok){setError(b.detail||"儲存失敗");return}setData(b);setEditing(false)};return <div className="card gap-alert-center"><header className="alert-section-head"><div><small>ON-CALL GAP NOTIFICATIONS</small><h2>值班缺口自動通知</h2><p>缺口進入提前提醒範圍時只通知一次，補班後標記為已恢復。</p></div>{canManage&&data&&<button className="create" onClick={()=>setEditing(true)}>通知設定</button>}</header>{error&&<div className="log-error">{error}</div>}{data&&<><div className="oncall-current"><span><small>通知功能</small><strong>{data.policy.alertEnabled?"啟用":"停用"}</strong></span><span><small>提前提醒</small><strong>{data.policy.alertLeadHours} 小時</strong></span><span><small>歷史紀錄</small><strong>{data.history.length}</strong></span></div><div className="data-table"><table><thead><tr><th>缺口範圍</th><th>通知狀態</th><th>成功管道</th><th>通知時間</th><th>恢復時間</th></tr></thead><tbody>{data.history.map((x:any)=><tr key={x.id}><td>{new Date(x.startsAt).toLocaleString("zh-TW",{hour12:false})}<small>至 {new Date(x.endsAt).toLocaleString("zh-TW",{hour12:false})}</small></td><td>{x.status}</td><td>{x.deliveryCount}</td><td>{new Date(x.notifiedAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{x.resolvedAt?new Date(x.resolvedAt).toLocaleString("zh-TW",{hour12:false}):"—"}</td></tr>)}</tbody></table></div>{!data.history.length&&<div className="empty-state"><strong>尚無值班缺口通知</strong></div>}{editing&&<div className="modal-shell"><form className="modal oncall-modal" onSubmit={save}><header><div><small>GAP NOTIFICATION POLICY</small><h2>調整缺口提醒</h2></div><button type="button" onClick={()=>setEditing(false)}>×</button></header><label className="inline-check"><input name="enabled" type="checkbox" defaultChecked={data.policy.alertEnabled}/><span>啟用值班缺口通知</span></label><label>提前提醒（小時）<input name="lead" type="number" min="1" max="168" defaultValue={data.policy.alertLeadHours}/></label><footer><button type="button" onClick={()=>setEditing(false)}>取消</button><button className="create">儲存設定</button></footer></form></div>}</>}</div>}

function RecurringOnCall({canManage}:{canManage:boolean}){const [data,setData]=useState<any>({users:[],templates:[]});const [open,setOpen]=useState(false);const [error,setError]=useState("");const load=useCallback(async()=>{const r=await fetch("/api/on-call-templates",{cache:"no-store"});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.detail||"載入失敗");setData(b);setError("")},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch("/api/on-call-templates",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:f.get("name"),userId:f.get("userId"),firstStartsAt:new Date(String(f.get("start"))).toISOString(),intervalDays:Number(f.get("interval")),durationMinutes:Number(f.get("duration")),horizonDays:Number(f.get("horizon"))})});const b=await r.json().catch(()=>({}));if(!r.ok){setError(b.detail||"建立失敗");return}setData(b);setOpen(false)};const remove=async(id:string)=>{const r=await fetch(`/api/on-call-templates/${id}`,{method:"DELETE"});if(!r.ok){setError("刪除失敗");return}await load()};const materialize=async()=>{const r=await fetch("/api/on-call-templates/materialize",{method:"POST"});const b=await r.json().catch(()=>({}));if(!r.ok){setError("展開失敗");return}setData(b)};return <div className="card recurring-oncall"><header className="alert-section-head"><div><small>RECURRING ON-CALL</small><h2>週期值班範本</h2><p>依首次開始、重複間隔與班次長度，自動展開未來排班。</p></div>{canManage&&<div><button onClick={()=>void materialize()}>立即展開</button><button className="create" onClick={()=>setOpen(true)}>＋ 新增範本</button></div>}</header>{error&&<div className="log-error">{error}</div>}<div className="data-table"><table><thead><tr><th>範本</th><th>值班人</th><th>重複</th><th>班次長度</th><th>展開範圍</th><th>未來班次</th><th>操作</th></tr></thead><tbody>{data.templates.map((x:any)=><tr key={x.id}><td><strong>{x.name}</strong><small>{new Date(x.firstStartsAt).toLocaleString("zh-TW",{hour12:false})}</small></td><td>{x.displayName}</td><td>每 {x.intervalDays} 天</td><td>{x.durationMinutes} 分鐘</td><td>{x.horizonDays} 天</td><td>{x.futureShifts}</td><td>{canManage&&<button onClick={()=>void remove(x.id)}>刪除</button>}</td></tr>)}</tbody></table></div>{!data.templates.length&&<div className="empty-state"><strong>尚無週期值班範本</strong></div>}{open&&<div className="modal-shell"><form className="modal oncall-modal" onSubmit={save}><header><div><small>RECURRING SHIFT TEMPLATE</small><h2>新增週期值班</h2></div><button type="button" onClick={()=>setOpen(false)}>×</button></header><label>範本名稱<input name="name" required defaultValue="每週值班"/></label><label>值班人員<select name="userId" required>{data.users.map((u:any)=><option key={u.id} value={u.id}>{u.displayName} · @{u.username}</option>)}</select></label><label>首次開始<input name="start" type="datetime-local" required/></label><label>重複間隔（天）<select name="interval" defaultValue="7"><option value="1">每天</option><option value="7">每週</option><option value="14">每兩週</option></select></label><label>班次長度（分鐘）<input name="duration" type="number" min="30" max="10080" defaultValue="480" required/></label><label>展開未來（天）<input name="horizon" type="number" min="1" max="180" defaultValue="30" required/></label><footer><button type="button" onClick={()=>setOpen(false)}>取消</button><button className="create">建立並展開</button></footer></form></div>}</div>}

function OnCallFairness({canManage}:{canManage:boolean}){const [data,setData]=useState<any|null>(null);const [editing,setEditing]=useState(false);const [error,setError]=useState("");const load=useCallback(async()=>{const r=await fetch("/api/on-call-fairness",{cache:"no-store"});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.detail||"載入失敗");setData(b);setError("")},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch("/api/on-call-fairness/policy",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({windowDays:Number(f.get("days")),imbalancePercent:Number(f.get("threshold"))})});const b=await r.json().catch(()=>({}));if(!r.ok){setError(b.detail||"儲存失敗");return}setData(b);setEditing(false)};return <div className="card fairness-center"><header className="alert-section-head"><div><small>ON-CALL FAIRNESS</small><h2>值班負載與公平性</h2><p>比較未來排班時數、目前告警、交接與責任 SLA 逾期。</p></div><div>{canManage&&<button onClick={()=>setEditing(true)}>調整政策</button>}<button className="create" onClick={()=>void load()}>重新計算</button></div></header>{error&&<div className="log-error">{error}</div>}{data&&<><div className="ownership-kpis"><span><small>觀察期間</small><strong>{data.policy.windowDays} 天</strong></span><span><small>排班總時數</small><strong>{data.summary.totalHours}</strong></span><span><small>平均時數</small><strong>{data.summary.averageHours}</strong></span><span><small>明顯不均</small><strong className={data.summary.imbalancedUsers?"warn":"ok"}>{data.summary.imbalancedUsers}</strong></span></div><div className="data-table"><table><thead><tr><th>使用者</th><th>排班時數</th><th>偏離平均</th><th>目前告警</th><th>交接流入／流出</th><th>SLA 逾期</th><th>狀態</th></tr></thead><tbody>{data.users.map((u:any)=><tr key={u.id}><td><strong>{u.displayName}</strong><small>@{u.username}</small></td><td>{u.scheduledHours}</td><td>{u.deviationPercent>0?"+":""}{u.deviationPercent}%</td><td>{u.activeAlerts}</td><td>{u.handoffsIn} / {u.handoffsOut}</td><td>{u.slaBreaches}</td><td><span className={`sla-state ${u.imbalanced?"overdue":"tracking"}`}>{u.imbalanced?"需平衡":"均衡"}</span></td></tr>)}</tbody></table></div>{editing&&<div className="modal-shell"><form className="modal oncall-modal" onSubmit={save}><header><div><small>FAIRNESS POLICY</small><h2>調整公平性門檻</h2></div><button type="button" onClick={()=>setEditing(false)}>×</button></header><label>觀察期間（天）<input name="days" type="number" min="1" max="180" defaultValue={data.policy.windowDays}/></label><label>允許偏離平均（%）<input name="threshold" type="number" min="1" max="100" step="0.01" defaultValue={data.policy.imbalancePercent}/></label><footer><button type="button" onClick={()=>setEditing(false)}>取消</button><button className="create">儲存政策</button></footer></form></div>}</>}</div>}

function OnCallHealth({canManage}:{canManage:boolean}){const [data,setData]=useState<any|null>(null);const [editing,setEditing]=useState(false);const [error,setError]=useState("");const load=useCallback(async()=>{const r=await fetch("/api/on-call-health",{cache:"no-store"});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.detail||"載入失敗");setData(b);setError("")},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch("/api/on-call-health/policy",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({horizonDays:Number(f.get("days")),maxShiftHours:Number(f.get("shift")),minRestHours:Number(f.get("rest")),maxWeeklyHours:Number(f.get("weekly"))})});const b=await r.json().catch(()=>({}));if(!r.ok){setError(b.detail||"儲存失敗");return}setData(b);setEditing(false)};const labels:any={overlap:"班次重疊",short_rest:"休息不足",long_shift:"單次過長",weekly_hours:"每週過高"};return <div className="card oncall-health"><header className="alert-section-head"><div><small>ON-CALL HEALTH CHECK</small><h2>值班健康檢查</h2><p>檢查重疊、休息不足、單次過長與每週工時風險。</p></div><div>{canManage&&<button onClick={()=>setEditing(true)}>調整門檻</button>}<button className="create" onClick={()=>void load()}>重新檢查</button></div></header>{error&&<div className="log-error">{error}</div>}{data&&<><div className="ownership-kpis"><span><small>觀察範圍</small><strong>{data.policy.horizonDays} 天</strong></span><span><small>班次</small><strong>{data.summary.shiftCount}</strong></span><span><small>風險</small><strong className={data.summary.issueCount?"warn":"ok"}>{data.summary.issueCount}</strong></span><span><small>重大</small><strong>{data.summary.criticalCount}</strong></span><span><small>整體</small><strong className={data.summary.healthy?"ok":"warn"}>{data.summary.healthy?"健康":"需調整"}</strong></span></div><div className="data-table"><table><thead><tr><th>使用者</th><th>類型</th><th>等級</th><th>時間</th><th>說明</th></tr></thead><tbody>{data.issues.map((x:any,i:number)=><tr key={`${x.type}-${i}`}><td><strong>{x.userName}</strong></td><td>{labels[x.type]}</td><td><span className={`sla-state ${x.severity==="critical"?"overdue":"due_soon"}`}>{x.severity==="critical"?"重大":"警告"}</span></td><td>{x.startsAt?new Date(x.startsAt).toLocaleString("zh-TW",{hour12:false}):"—"}</td><td>{x.detail}</td></tr>)}</tbody></table></div>{!data.issues.length&&<div className="empty-state"><strong>沒有發現值班健康風險</strong></div>}{editing&&<div className="modal-shell"><form className="modal oncall-modal" onSubmit={save}><header><div><small>ON-CALL SAFETY POLICY</small><h2>調整健康門檻</h2></div><button type="button" onClick={()=>setEditing(false)}>×</button></header><label>觀察範圍（天）<input name="days" type="number" min="1" max="90" defaultValue={data.policy.horizonDays}/></label><label>單次最長（小時）<input name="shift" type="number" min="1" max="168" step="0.5" defaultValue={data.policy.maxShiftHours}/></label><label>最少休息（小時）<input name="rest" type="number" min="0" max="72" step="0.5" defaultValue={data.policy.minRestHours}/></label><label>每週上限（小時）<input name="weekly" type="number" min="1" max="168" step="0.5" defaultValue={data.policy.maxWeeklyHours}/></label><footer><button type="button" onClick={()=>setEditing(false)}>取消</button><button className="create">儲存門檻</button></footer></form></div>}</>}</div>}

function AlertOwnershipQueue({canManage}:{canManage:boolean}){
 const [data,setData]=useState<any>({users:[],events:[],workload:[],history:[]});const [selected,setSelected]=useState<any|null>(null);const [error,setError]=useState("");const responseBody=async(r:Response)=>{const text=await r.text();try{return JSON.parse(text)}catch{return {detail:r.ok?"伺服器回應格式錯誤":`伺服器錯誤（HTTP ${r.status}）`}}};const load=useCallback(async()=>{const r=await fetch("/api/alert-ownership",{cache:"no-store"});const b=await responseBody(r);if(!r.ok)throw new Error(b.detail||"載入失敗");setData(b);setError("")},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);
 const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();if(!selected)return;const d=new FormData(e.currentTarget);const r=await fetch(`/api/alert-ownership/${selected.id}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({userId:d.get("userId")||null,note:d.get("note")})});const b=await responseBody(r);if(!r.ok){setError(b.detail||"指派失敗");return}setData(b);setError("");setSelected(null)};const unassigned=data.events.filter((e:any)=>!e.assigneeId).length;
 return <div className="card ownership-center"><header className="alert-section-head"><div><small>ACTIVE ALERT OWNERSHIP</small><h2>告警負責人工作佇列</h2><p>人工指派、轉派或解除負責人；所有異動保留完整歷史。</p></div><button className="create" onClick={()=>void load()}>重新整理</button></header>{error&&<div className="log-error">{error}</div>}<div className="ownership-kpis"><span><small>作用中告警</small><strong>{data.events.length}</strong></span><span><small>未指派</small><strong className={unassigned?"warn":"ok"}>{unassigned}</strong></span>{data.workload.filter((w:any)=>w.count>0).slice(0,4).map((w:any)=><span key={w.userId}><small>{w.displayName}</small><strong>{w.count}</strong></span>)}</div><div className="data-table"><table><thead><tr><th>主機／規則</th><th>等級</th><th>狀態</th><th>目前負責人</th><th>開始時間</th><th>操作</th></tr></thead><tbody>{data.events.map((item:any)=><tr key={item.id}><td><strong>{item.hostName}</strong><small>{item.ruleName}</small></td><td>{item.severity}</td><td>{item.status}</td><td><span className={`ownership-state ${item.assigneeId?"assigned":"unassigned"}`}>{item.assigneeName||"未指派"}</span></td><td>{new Date(item.startedAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{canManage&&<button onClick={()=>setSelected(item)}>{item.assigneeId?"轉派":"指派"}</button>}</td></tr>)}</tbody></table></div>{!data.events.length&&<div className="empty-state"><strong>目前沒有作用中告警</strong></div>}{data.history.length>0&&<div className="ownership-history">{data.history.slice(0,10).map((item:any)=><span key={item.id}><strong>{item.action==="assign"?"指派":item.action==="reassign"?"轉派":"解除"}</strong><small>{item.previousUserName?`${item.previousUserName} → `:""}{item.userName||"未指派"} · {item.note||"無備註"}</small><time>{new Date(item.assignedAt).toLocaleString("zh-TW",{hour12:false})}</time></span>)}</div>}{selected&&<div className="modal-shell"><form className="modal ownership-modal" onSubmit={save}><header><div><small>{selected.hostName} · {selected.ruleName}</small><h2>更新告警負責人</h2></div><button type="button" onClick={()=>setSelected(null)}>×</button></header><label>負責人<select name="userId" defaultValue={selected.assigneeId||""}><option value="">解除指派</option>{data.users.map((u:any)=><option key={u.id} value={u.id}>{u.displayName} · @{u.username}</option>)}</select></label><label>異動備註<textarea name="note" placeholder="說明指派、轉派或解除原因" maxLength={500}/></label><footer><button type="button" onClick={()=>setSelected(null)}>取消</button><button className="create">確認異動</button></footer></form></div>}</div>
}

function AlertOwnershipSla({canManage}:{canManage:boolean}){
 const [data,setData]=useState<any|null>(null);const [editing,setEditing]=useState(false);const [error,setError]=useState("");const parse=async(r:Response)=>{const t=await r.text();try{return JSON.parse(t)}catch{return {detail:`伺服器錯誤（HTTP ${r.status}）`}}};const load=useCallback(async()=>{const r=await fetch("/api/alert-ownership-sla",{cache:"no-store"});const b=await parse(r);if(!r.ok)throw new Error(b.detail||"載入失敗");setData(b);setError("")},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);const r=await fetch("/api/alert-ownership-sla/policy",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({warningMinutes:Number(f.get("warning")),criticalMinutes:Number(f.get("critical")),unassignedMinutes:Number(f.get("unassigned")),dueSoonPercent:Number(f.get("soon"))})});const b=await parse(r);if(!r.ok){setError(b.detail||"儲存失敗");return}setData(b);setEditing(false)};const state:any={overdue:"已逾期",due_soon:"即將逾期",tracking:"追蹤中",acknowledged:"已確認"};return <div className="card ownership-sla"><header className="alert-section-head"><div><small>ALERT OWNERSHIP SLA</small><h2>告警責任時效</h2><p>依事件開始時間追蹤確認期限與未指派逾時。</p></div><div>{canManage&&<button onClick={()=>setEditing(true)}>調整政策</button>}<button className="create" onClick={()=>void load()}>重新整理</button></div></header>{error&&<div className="log-error">{error}</div>}{data&&<><div className="ownership-kpis"><span><small>作用中</small><strong>{data.summary.active}</strong></span><span><small>已逾期</small><strong className={data.summary.overdue?"warn":"ok"}>{data.summary.overdue}</strong></span><span><small>即將逾期</small><strong>{data.summary.dueSoon}</strong></span><span><small>未指派逾時</small><strong>{data.summary.unassignedOverdue}</strong></span></div><div className="data-table"><table><thead><tr><th>主機／規則</th><th>等級</th><th>負責人</th><th>期限</th><th>SLA 狀態</th></tr></thead><tbody>{data.events.map((x:any)=><tr key={x.id}><td><strong>{x.hostName}</strong><small>{x.ruleName}</small></td><td>{x.severity}</td><td>{x.assigneeName||"未指派"}</td><td>{new Date(x.deadlineAt).toLocaleString("zh-TW",{hour12:false})}</td><td><span className={`sla-state ${x.state}`}>{state[x.state]}{x.unassignedOverdue?" · 未指派":""}</span></td></tr>)}</tbody></table></div>{editing&&<div className="modal-shell"><form className="modal ownership-modal" onSubmit={save}><header><div><small>OWNERSHIP OBJECTIVE</small><h2>調整責任 SLA</h2></div><button type="button" onClick={()=>setEditing(false)}>×</button></header><label>警告確認期限（分鐘）<input name="warning" type="number" min="1" defaultValue={data.policy.warningMinutes}/></label><label>重大確認期限（分鐘）<input name="critical" type="number" min="1" defaultValue={data.policy.criticalMinutes}/></label><label>未指派逾時（分鐘）<input name="unassigned" type="number" min="1" defaultValue={data.policy.unassignedMinutes}/></label><label>即將逾期門檻（剩餘百分比）<input name="soon" type="number" min="5" max="90" defaultValue={data.policy.dueSoonPercent}/></label><footer><button type="button" onClick={()=>setEditing(false)}>取消</button><button className="create">儲存政策</button></footer></form></div>}</>}</div>
}

function AlertSlaEscalationHistory(){const [items,setItems]=useState<any[]>([]);useEffect(()=>{void fetch("/api/alert-ownership-sla",{cache:"no-store"}).then(r=>r.ok?r.json():Promise.reject()).then(b=>setItems(b.history||[])).catch(()=>setItems([]))},[]);return <div className="card sla-history"><header className="alert-section-head"><div><small>SLA ESCALATION HISTORY</small><h2>責任逾期升級紀錄</h2><p>每個事件只發送一次逾期升級，確認或恢復後標記為已恢復。</p></div></header><div className="data-table"><table><thead><tr><th>主機／規則</th><th>逾期原因</th><th>通知狀態</th><th>成功管道</th><th>逾期時間</th><th>恢復時間</th></tr></thead><tbody>{items.map(x=><tr key={x.id}><td><strong>{x.hostName}</strong><small>{x.ruleName}</small></td><td>{x.breachType==="unassigned"?"未指派逾時":"確認期限逾時"}</td><td>{x.status}</td><td>{x.deliveryCount}</td><td>{new Date(x.breachedAt).toLocaleString("zh-TW",{hour12:false})}</td><td>{x.recoveredAt?new Date(x.recoveredAt).toLocaleString("zh-TW",{hour12:false}):"—"}</td></tr>)}</tbody></table></div>{!items.length&&<div className="empty-state"><strong>尚無責任 SLA 升級</strong></div>}</div>}
...
function NotificationDeliveryHealth({canManage}:{canManage:boolean}){
 const [report,setReport]=useState<any|null>(null);const [editing,setEditing]=useState(false);const [error,setError]=useState("");
 const load=useCallback(async()=>{const r=await fetch("/api/notification-delivery-health",{cache:"no-store"});const b=await r.json();if(!r.ok)throw new Error(b.detail||"載入失敗");setReport(b)},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);
 const save=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const d=new FormData(e.currentTarget);const r=await fetch("/api/notification-delivery-health/policy",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({windowDays:Number(d.get("windowDays")),successTarget:Number(d.get("successTarget")),minimumSamples:Number(d.get("minimumSamples"))})});const b=await r.json();if(!r.ok){setError(b.detail||"儲存失敗");return}setReport(b);setEditing(false)};
 const names:any={telegram:"Telegram",line:"LINE",sms:"SMS Gateway",webhook:"Webhook",email:"Email"};
 return <div className="card delivery-health"><header className="alert-section-head"><div><small>NOTIFICATION DELIVERY SLO</small><h2>通知交付健康度</h2><p>排除隔離測試與治理抑制，依實際發送嘗試計算成功率。</p></div><div className="reliability-actions">{canManage&&<button className="secondary-action" onClick={()=>setEditing(true)}>調整目標</button>}<button className="create" onClick={()=>void load()}>重新計算</button></div></header>{error&&<div className="log-error">{error}</div>}{report&&<><div className="delivery-health-kpis"><span><small>觀察期間</small><strong>{report.policy.windowDays} 天</strong></span><span><small>成功率目標</small><strong>{report.policy.successTarget}%</strong></span><span><small>整體成功率</small><strong className={report.summary.met?"ok":"warn"}>{report.summary.successRate==null?"資料不足":`${report.summary.successRate}%`}</strong></span><span><small>成功／失敗</small><strong>{report.summary.sent} / {report.summary.failed}</strong></span><span><small>補送成功／耗盡</small><strong>{report.summary.recoveredRetries} / {report.summary.exhaustedRetries}</strong></span></div><div className="data-table"><table><thead><tr><th>管道</th><th>樣本</th><th>成功</th><th>失敗</th><th>抑制</th><th>成功率</th><th>狀態</th><th>最近失敗</th></tr></thead><tbody>{report.channels.map((item:any)=><tr key={item.channel}><td><strong>{names[item.channel]||item.channel}</strong></td><td>{item.attempts}</td><td>{item.sent}</td><td>{item.failed}</td><td>{item.suppressed}</td><td>{item.successRate==null?"—":`${item.successRate}%`}</td><td><span className={`slo-state ${item.met?"met":"missed"}`}>{!item.enoughSamples?"資料不足":item.met?"達標":"未達標"}</span></td><td>{item.lastFailedAt?new Date(item.lastFailedAt).toLocaleString("zh-TW",{hour12:false}):"—"}</td></tr>)}</tbody></table></div></>}{editing&&report&&<div className="modal-shell"><form className="modal delivery-health-policy" onSubmit={save}><header><div><small>DELIVERY OBJECTIVE</small><h2>調整通知交付目標</h2></div><button type="button" onClick={()=>setEditing(false)}>×</button></header><label>觀察天數<input name="windowDays" type="number" min="1" max="90" defaultValue={report.policy.windowDays}/></label><label>成功率目標（%）<input name="successTarget" type="number" min="50" max="100" step="0.01" defaultValue={report.policy.successTarget}/></label><label>最低樣本數<input name="minimumSamples" type="number" min="1" max="1000" defaultValue={report.policy.minimumSamples}/></label><footer><button type="button" onClick={()=>setEditing(false)}>取消</button><button className="create">儲存目標</button></footer></form></div>}</div>
}
...
function Alerts({
  ...
  return (
    <section className="alerts-page">
      ...
      <MaintenanceWindows canManage={canManage} hosts={hosts} rules={rules} />
      <AlertInhibitions canManage={canManage} rules={rules} />
      <RootCauseCorrelations />
      <AlertStormProtection canManage={canManage} />
      <OnCallSchedule canManage={canManage} />
      <OnCallHandoffs canManage={canManage} />
      <RecurringOnCall canManage={canManage} />
      <OnCallFairness canManage={canManage} />
      <OnCallHealth canManage={canManage} />
      <OnCallCoverage canManage={canManage} />
      <OnCallGapAlerts canManage={canManage} />
      <AlertOwnershipQueue canManage={canManage} />
      <AlertOwnershipSla canManage={canManage} />
      <AlertSlaEscalationHistory />
      <NotificationEscalation canManage={canManage} />
      <NotificationRouting canManage={canManage} hosts={hosts} rules={rules} />
      <NotificationDeliveryHealth canManage={canManage} />
