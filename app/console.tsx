...
function OnCallSchedule({canManage}:{canManage:boolean}){
 const [data,setData]=useState<any>({users:[],shifts:[],assignments:[]});const [open,setOpen]=useState(false);const [error,setError]=useState("");const load=useCallback(async()=>{const r=await fetch("/api/on-call",{cache:"no-store"});const b=await r.json();if(!r.ok)throw new Error(b.detail||"載入失敗");setData(b)},[]);useEffect(()=>{void load().catch(e=>setError(e.message))},[load]);
 const create=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const d=new FormData(e.currentTarget);const r=await fetch("/api/on-call/shifts",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({userId:d.get("userId"),startsAt:new Date(String(d.get("startsAt"))).toISOString(),endsAt:new Date(String(d.get("endsAt"))).toISOString(),note:d.get("note"),enabled:true})});const b=await r.json();if(!r.ok){setError(b.detail||"建立失敗");return}setData(b);setOpen(false)};const remove=async(id:string)=>{const r=await fetch(`/api/on-call/shifts/${id}`,{method:"DELETE"});if(!r.ok){setError("刪除失敗");return}await load()};const current=data.shifts.find((s:any)=>s.active);
