...
docker exec -i "$container" psql -v ON_ERROR_STOP=1 -U postgres -d linux_ai < backend/migrations/026_recurring_on_call.sql >/dev/null
docker exec -i "$container" psql -v ON_ERROR_STOP=1 -U postgres -d linux_ai < backend/migrations/027_on_call_fairness.sql >/dev/null
docker exec -i "$container" psql -v ON_ERROR_STOP=1 -U postgres -d linux_ai < backend/migrations/028_on_call_health.sql >/dev/null
...
docker exec "$container" psql -At -U postgres -d linux_ai -c "SELECT to_regclass('public.on_call_templates') IS NOT NULL" | grep -qx t
docker exec "$container" psql -At -U postgres -d linux_ai -c "SELECT to_regclass('public.on_call_fairness_policy') IS NOT NULL" | grep -qx t
docker exec "$container" psql -At -U postgres -d linux_ai -c "SELECT to_regclass('public.on_call_health_policy') IS NOT NULL" | grep -qx t
docker exec -i "$container" psql -v ON_ERROR_STOP=1 -U postgres -d linux_ai < backend/migrations/down/028_on_call_health.sql >/dev/null
docker exec -i "$container" psql -v ON_ERROR_STOP=1 -U postgres -d linux_ai < backend/migrations/down/027_on_call_fairness.sql >/dev/null
docker exec -i "$container" psql -v ON_ERROR_STOP=1 -U postgres -d linux_ai < backend/migrations/down/026_recurring_on_call.sql >/dev/null
...
echo "Migration 001 → 028 與 rollback 測試通過"
